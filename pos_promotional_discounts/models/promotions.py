...
class PosPromotions(models.Model):
    ...
    offer_type = fields.Selection([('discount_on_products','Discount on Products'),    
                                    ...
                                    # ('buy_combo_get_product_free', 'Buy Combo Products Get a Product Free'),
                                    ], required=True)
    ...
    combo_offer_products_ids = fields.One2many('combo.products', 'combo_product_id', string="Combo Products")
    ...
    def _default_discount_product_id(self):
        if self and self.offer_type == 'get_x_discount_on_sale_total':
            product = self.env.ref("point_of_sale.product_product_consumable", raise_if_not_found=False)
            self.discount_product_id = product or False
...
class DiscountProducts(models.Model):
    ...
    sequence1 = fields.Integer(string="Sequence") 
...
class DiscountedProducts(models.Model):
    _name = "discounted.products"

    discount_product_id = fields.Many2one('pos.promotions', string="Discounted Product")
    product_id = fields.Many2one('product.product', domain=[('available_in_pos', '=', True)])
    discount_type = fields.Selection([('fixed','Fixed'), ('percent', 'Percent')], string="Discount Type")
    discount = fields.Integer("Discount %")
    product_list_price = fields.Integer("Display Price", compute='_compute_list_price')
    product_discounted_price = fields.Integer("Discounted Price", compute='_compute_discounted_price')

    @api.depends('product_id')
    def _compute_list_price(self):
        for record in self:
            if record.product_id:
                record.product_list_price = record.product_id.list_price
            else:
                record.product_list_price = 0

    @api.depends('product_id', 'discount')
    def _compute_discounted_price(self):
        for record in self:
            if record.product_id and record.discount:
                record.product_discounted_price = (record.product_id.list_price - ((record.product_id.list_price * record.discount)/100))
            else:
                record.product_discounted_price = 0
...
class BuyComboProducts(models.Model):
    _name = 'combo.products'

    combo_product_id = fields.Many2one('pos.promotions', string="Get Free Product On Buying Combo")
    combo_product_ids = fields.Many2many('product.product', domain=[('available_in_pos', '=', True)], string="Combo Products")
    free_product_id = fields.Many2one('product.product', domain=[('available_in_pos', '=', True)], string="Free Products")
